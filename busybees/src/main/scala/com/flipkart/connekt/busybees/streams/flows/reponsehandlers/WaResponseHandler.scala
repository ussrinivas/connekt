...
import com.flipkart.connekt.busybees.models.{GCMRequestTracker, SmsRequestTracker, SmsResponse, WaRequestTracker}
...
class WaResponseHandler(implicit m: Materializer, ec: ExecutionContext) extends  WaProviderResponseHandler[(Try[SmsResponse], WaRequestTracker)](90) with Instrumented {
...
//              val contactNosItr = contactNos.iterator
//
//              responseBody.findValue("payload").foreach(rBlock => {
//                val rDeviceId = contactNosItr.next()
//                rBlock match {
//                  case s if s.has("message_id") =>
//                    if (s.has("registration_id"))
//                      DeviceDetailsService.get(appName, rDeviceId).foreach(_.foreach(d => {
//                        ConnektLogger(LogFile.PROCESSORS).info(s"WaResponseHandler device token update notified on. $messageId of device: $rDeviceId")
...
//                        ConnektLogger(LogFile.PROCESSORS).info(s"WaResponseHandler device token invalid / not_found, deleting details of device: $rDeviceId.")
...
//                  case e: JsonNode =>
//                    ConnektLogger(LogFile.PROCESSORS).error(s"WaResponseHandler unknown for message: $messageId, device: $rDeviceId", e)
...
//              ConnektLogger(LogFile.PROCESSORS).error(s"WaResponseHandler http response - invalid json sent for: $messageId response: $stringResponse")
//            case 401 =>
...
//              ConnektLogger(LogFile.PROCESSORS).error(s"WaResponseHandler http response - the sender account used to send a message couldn't be authenticated for: $messageId response: $stringResponse")
//            case w if 5 == (w / 100) =>
...
//              ConnektLogger(LogFile.PROCESSORS).error(s"WaResponseHandler http response - the gcm server encountered an error while trying to process the request for: $messageId code: $w response: $stringResponse")
//            case w =>
...
//              ConnektLogger(LogFile.PROCESSORS).error(s"WaResponseHandler http response - gcm response unhandled for: $messageId code: $w response: $stringResponse")
//          }
//        } catch {
//          case e: Exception =>
...
//            ConnektLogger(LogFile.PROCESSORS).error(s"WaResponseHandler failed processing http response body for: $messageId", e)
//        }
//      case Failure(e2) =>
...
//        ConnektLogger(LogFile.PROCESSORS).error(s"WaResponseHandler gcm send failure for: $messageId", e2)
//    }
