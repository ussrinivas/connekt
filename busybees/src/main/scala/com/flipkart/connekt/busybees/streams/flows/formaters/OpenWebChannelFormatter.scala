...
class OpenWebChannelFormatter(parallelism: Int)(implicit ec: ExecutionContextExecutor) extends NIOFlow[ConnektRequest, OpenWebStandardPayloadEnvelope](parallelism)(ec) {
  ...
  override def map: ConnektRequest => List[OpenWebStandardPayloadEnvelope] = message => {
    ...
    try {
      ...
      invalidDeviceIds.map(PNCallbackEvent(message.id, _, InternalStatus.MissingDeviceInfo, MobilePlatform.OPENWEB, pnInfo.appName, message.contextId.orEmpty, message.client)).persist
      ...
      if (ttl > 0) {
          ...
          if (device.token != null && device.token.nonEmpty && device.token.isValidUrl) {
            ...
            if (device.keys != null && device.keys.nonEmpty) {
              Try(WebPushEncryptionUtils.encrypt(device.keys("p256dh"), device.keys("auth"), appDataWithId)) match {
                case Success(data) =>
                  ...
                  List(OpenWebStandardPayloadEnvelope(message.id, device.deviceId, pnInfo.appName,
                    message.contextId.orEmpty, message.client, token, OpenWebStandardPayload(data.encodedData), headers.toMap, message.meta))
                case Failure(e) =>
              ...
              List(OpenWebStandardPayloadEnvelope(message.id, device.deviceId, pnInfo.appName, message.contextId.orEmpty,
                message.client, token, OpenWebStandardPayload(Array.empty), headers.toMap, message.meta))
            }
          } else {
        ...
        devicesInfo.map(d => PNCallbackEvent(message.id, d.deviceId, InternalStatus.TTLExpired, MobilePlatform.OPENWEB, d.appName, message.contextId.orEmpty, message.client)).persist
        List.empty[OpenWebStandardPayloadEnvelope]
      }
    }
    catch {
      case e: Exception =>
        ConnektLogger(LogFile.PROCESSORS).error(s"OpenWebChannelFormatter error for ${message.id}", e)
        throw new ConnektPNStageException(message.id, message.deviceId, InternalStatus.StageError, message.appName, message.platform, message.contextId.orEmpty, message.client, message.meta, "OpenWebChannelFormatter::".concat(e.getMessage), e)
    }
  }
