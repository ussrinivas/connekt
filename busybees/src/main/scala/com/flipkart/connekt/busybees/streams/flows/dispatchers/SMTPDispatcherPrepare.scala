...
class SMTPDispatcherPrepare extends MapFlowStage[EmailPayloadEnvelope, (EmailPayloadEnvelope, EmailRequestTracker)] {
  ...
  override implicit val map: EmailPayloadEnvelope => List[(EmailPayloadEnvelope, EmailRequestTracker)] = message => {
    ...
    val requestTrace = EmailRequestTracker(messageId = message.messageId, clientId = message.clientId, to = message.payload.to.map(_.address), cc = message.payload.cc.map(_.address), appName = message.appName, contextId = message.contextId, meta = message.meta)
