  ...
  override val map: (ConnektRequest) => (List[(HttpRequest, WAMediaRequestTracker)]) = connektRequest => profile("map") {
    try {
      ...
      case e: Exception =>
        ...
        throw ConnektStageException(connektRequest.id, connektRequest.clientId, connektRequest.destinations, InternalStatus.StageError, connektRequest.appName, Channel.WA, connektRequest.contextId.orEmpty, connektRequest.meta, "WAMediaDispatcher::".concat(e.toString), e)
    }
  }
