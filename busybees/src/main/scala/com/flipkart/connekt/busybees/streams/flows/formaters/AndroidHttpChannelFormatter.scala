...
class AndroidHttpChannelFormatter (parallelism: Int)(implicit ec: ExecutionContextExecutor) extends FCMChannelFormatter(parallelism)(ec) {
  ...
  override def createPayload(message:ConnektRequest,
    ...
    List(GCMPayloadEnvelope(message.id, message.clientId, validDeviceIds, pnInfo.appName, platform = pnInfo.platform, message.contextId.orEmpty, payload, message.meta))
  }
}
