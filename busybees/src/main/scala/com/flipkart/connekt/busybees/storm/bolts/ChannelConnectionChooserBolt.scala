...
/**
  ...
  */
class ChannelConnectionChooserBolt extends BaseBasicBolt {
  private lazy val xmppClients = ConnektConfig.getList[String]("android.protocol.xmpp-clients")

  override def execute(input: Tuple, collector: BasicOutputCollector): Unit = {
    val connektRequest = input.asInstanceOf[TupleImpl].get("filteredMessage").asInstanceOf[ConnektRequest]
  ...
  override def declareOutputFields(declarer: OutputFieldsDeclarer): Unit = {
    declarer.declareStream(AndroidProtocols.http, new Fields("connektRequest"))
    declarer.declareStream(AndroidProtocols.xmpp, new Fields("connektRequest"))
  }
}
