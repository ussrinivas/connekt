...
class IOSChannelFormatter(parallelism: Int)(implicit ec: ExecutionContextExecutor) extends NIOFlow[ConnektRequest, APSPayloadEnvelope](parallelism)(ec) {
  ...
  override def map: (ConnektRequest) => List[APSPayloadEnvelope] = message => {
    try {
      ...
      invalidDeviceIds.map(PNCallbackEvent(message.id, _, InternalStatus.MissingDeviceInfo, MobilePlatform.IOS, pnInfo.appName, message.contextId.orEmpty, message.client)).persist
      ...
      if (apnsEnvelopes.nonEmpty && ttlInMillis > System.currentTimeMillis()) {
        ...
        apnsEnvelopes.map(e => PNCallbackEvent(e.messageId, e.deviceId, InternalStatus.TTLExpired, MobilePlatform.IOS, e.appName, message.contextId.orEmpty, message.client)).persist
        List.empty[APSPayloadEnvelope]
      } else
        List.empty[APSPayloadEnvelope]
      ...
      case e: Throwable =>
        ...
        throw new ConnektPNStageException(message.id, message.deviceId, InternalStatus.StageError, message.appName, message.platform, message.contextId.orEmpty, message.client, message.meta, "IOSChannelFormatter::".concat(e.getMessage), e)
    }
  }
}
