...
//          ConnektLogger(LogFile.PROCESSORS).debug(s"IOSChannelFormatter dropping dry-run message: ${message.id}")
      ...
      case e: Throwable =>
        ...
        throw new ConnektChannelStageException(message.id, message.clientId,Channel.PUSH, message.destinations, InternalStatus.StageError, message.appName, message.platform, message.contextId.orEmpty, message.meta, "IOSChannelFormatter::".concat(e.getMessage), e)
    }
  }
}
