...
//      ConnektLogger(LogFile.PROCESSORS).trace(s"OpenWebDispatcherPrepare received message: ${message.toString}")
      ...
      case e: Throwable =>
        ...
        throw new ConnektStageException(message.messageId, message.clientId, Channel.PUSH, Set(message.deviceId), InternalStatus.StageError, message.appName, MobilePlatform.OPENWEB, message.contextId, message.meta, s"OpenWebDispatcherPrepare-${e.getMessage}", e)
    }
  }
}
