...
class WNSResponseHandler(implicit m: Materializer, ec: ExecutionContext) extends PNProviderResponseErrorHandler[(Try[HttpResponse], WNSRequestTracker), WNSRequestTracker] {
  ...
  private def handleWNSResponse(tryResponse: Try[HttpResponse], requestTracker: WNSRequestTracker): Option[PNCallbackEvent] = {
    ...
    val maybePNCallbackEvent: Option[PNCallbackEvent] = tryResponse match {
      case Success(r) =>
        ...
        Option(r.status.intValue() match {
          case 200 =>
            ...
            ServiceFactory.getReportingService.recordPushStatsDelta(requestTracker.request.client, Option(contextId), requestTracker.meta.get("stencilId").map(_.toString), Option(requestTracker.appName), MobilePlatform.WINDOWS, WNSResponseStatus.Received)
          ...
          case 400 =>
            ServiceFactory.getReportingService.recordPushStatsDelta(requestTracker.request.client, Option(contextId), requestTracker.meta.get("stencilId").map(_.toString), Option(requestTracker.appName), MobilePlatform.WINDOWS, WNSResponseStatus.InvalidHeader)
          ...
          case 401 =>
            ServiceFactory.getReportingService.recordPushStatsDelta(requestTracker.request.client, Option(contextId), requestTracker.meta.get("stencilId").map(_.toString), Option(requestTracker.appName), MobilePlatform.WINDOWS, WNSResponseStatus.AuthError)
            ...
            null
          case 403 =>
            ServiceFactory.getReportingService.recordPushStatsDelta(requestTracker.request.client, Option(contextId), requestTracker.meta.get("stencilId").map(_.toString), Option(requestTracker.appName), MobilePlatform.WINDOWS, WNSResponseStatus.InvalidMethod)
          ...
          case 404 =>
            DeviceDetailsService.get(appName, requestTracker.request.deviceId).transform[PNCallbackEvent]({
              case Some(dd) if dd.osName == "windows" =>
                ServiceFactory.getReportingService.recordPushStatsDelta( requestTracker.request.client  ,Option(contextId), requestTracker.meta.get("stencilId").map(_.toString), Option(requestTracker.appName) , MobilePlatform.WINDOWS , WNSResponseStatus.InvalidChannelUri)
              ...
              case Some(dd)  =>
                ServiceFactory.getReportingService.recordPushStatsDelta( requestTracker.request.client  ,Option(contextId), requestTracker.meta.get("stencilId").map(_.toString), Option(requestTracker.appName) , MobilePlatform.WINDOWS , WNSResponseStatus.InvalidDevice)
          ...
          case 405 =>
            ...
            PNCallbackEvent(requestId, deviceId, WNSResponseStatus.InvalidMethod, MobilePlatform.WINDOWS, appName, contextId, client, r.optHeader("X-WNS-MSG-ID"), eventTS)
          case 406 =>
            ServiceFactory.getReportingService.recordPushStatsDelta(requestTracker.request.client, Option(contextId), requestTracker.meta.get("stencilId").map(_.toString), Option(requestTracker.appName), MobilePlatform.WINDOWS, WNSResponseStatus.ThrottleLimitExceeded)
          ...
          case 410 =>
            ...
            DeviceDetailsService.get(appName, requestTracker.request.deviceId).transform[PNCallbackEvent]({
              case Some(dd) if dd.osName == "windows" =>
                ...
                ServiceFactory.getReportingService.recordPushStatsDelta( requestTracker.request.client  ,Option(contextId), requestTracker.meta.get("stencilId").map(_.toString), Option(requestTracker.appName) , MobilePlatform.WINDOWS , WNSResponseStatus.ChannelExpired)
          ...
          case 412 =>
            ServiceFactory.getReportingService.recordPushStatsDelta(requestTracker.request.client, Option(contextId), requestTracker.meta.get("stencilId").map(_.toString), Option(requestTracker.appName), MobilePlatform.WINDOWS, WNSResponseStatus.PreConditionFailed)
          ...
          case 413 =>
            ServiceFactory.getReportingService.recordPushStatsDelta(requestTracker.request.client, Option(contextId), requestTracker.meta.get("stencilId").map(_.toString), Option(requestTracker.appName), MobilePlatform.WINDOWS, WNSResponseStatus.EntityTooLarge)
          ...
          case w if 5 == (w / 100) =>
            ServiceFactory.getReportingService.recordPushStatsDelta(requestTracker.request.client, Option(contextId), requestTracker.meta.get("stencilId").map(_.toString), Option(requestTracker.appName), MobilePlatform.WINDOWS, WNSResponseStatus.InternalError)
      ...
      case Failure(e) =>
        ServiceFactory.getReportingService.recordPushStatsDelta(requestTracker.request.client, Option(contextId), requestTracker.meta.get("stencilId").map(_.toString), Option(requestTracker.appName), MobilePlatform.WINDOWS, InternalStatus.ProviderSendError)
