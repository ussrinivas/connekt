  ...
  private def handleSmsResponse(tryResponse: Try[SmsResponse], requestTracker: SmsRequestTracker): Either[List[SmsCallbackEvent], List[SmsCallbackEvent]] = {
    ...
    val maybeSmsCallbackEvent = tryResponse match {
      ...
      case Failure(e) =>
        ...
        Left(receivers.map(SmsCallbackEvent(requestTracker.messageId, "", InternalStatus.ProviderSendError, _,
          requestTracker.clientId, requestTracker.provider, requestTracker.appName, Channel.SMS, requestTracker.contextId, s"SmsResponseHandler-${e.getClass.getSimpleName}-${e.getMessage}")).toList)
    }
