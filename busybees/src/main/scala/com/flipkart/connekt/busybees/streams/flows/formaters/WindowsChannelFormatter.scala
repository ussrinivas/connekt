...
//          ConnektLogger(LogFile.PROCESSORS).debug(s"WindowsChannelFormatter dropping dry-run message: ${message.id}")
      ...
      case e: Exception =>
        ...
        throw new ConnektChannelStageException(message.id, message.clientId, Channel.PUSH,message.destinations, InternalStatus.StageError, message.appName, message.platform, message.contextId.orEmpty, message.meta, "WindowsChannelFormatter::".concat(e.getMessage), e)
    }
  }
}
