...
class WindowsChannelFormatter(parallelism: Int)(implicit ec: ExecutionContextExecutor) extends NIOFlow[ConnektRequest, WNSPayloadEnvelope](parallelism)(ec) {
  ...
  override def map: (ConnektRequest) => List[WNSPayloadEnvelope] = message => {
    ...
    try {
      ...
      invalidDeviceIds.map(PNCallbackEvent(message.id, message.client, _, InternalStatus.MissingDeviceInfo, MobilePlatform.WINDOWS, pnInfo.appName, message.contextId.orEmpty)).persist
      ...
      if (wnsRequestEnvelopes.nonEmpty && ttlInSeconds > 0) {
        ...
        wnsRequestEnvelopes.map(w => PNCallbackEvent(w.messageId, message.client, w.deviceId, InternalStatus.TTLExpired, MobilePlatform.WINDOWS, pnInfo.appName, message.contextId.orEmpty)).persist
        List.empty[WNSPayloadEnvelope]
      } else
        List.empty[WNSPayloadEnvelope]
    } catch {
      case e: Exception =>
        ...
        throw new ConnektPNStageException(message.id, message.client, message.deviceId, InternalStatus.StageError, message.appName, message.platform, message.contextId.orEmpty, message.meta, "WindowsChannelFormatter::".concat(e.getMessage), e)
    }
  }
}
