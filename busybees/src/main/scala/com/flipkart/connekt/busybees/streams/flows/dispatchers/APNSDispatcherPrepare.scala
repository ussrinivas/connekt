  ...
  override val map: APSPayloadEnvelope => List[(SimpleApnsPushNotification, APNSRequestTracker)] = envelope => {
    try {
      ...
      case e: Throwable =>
        ...
        throw new ConnektPNStageException(envelope.messageId, Set(envelope.deviceId), InternalStatus.StageError, envelope.appName, MobilePlatform.IOS, envelope.contextId, s"APNSDispatcherPrepare-${e.getMessage}", e)
    }
  }
}
