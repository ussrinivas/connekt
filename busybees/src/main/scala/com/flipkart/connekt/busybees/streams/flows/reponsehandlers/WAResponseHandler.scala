        ...
        case Success(r) =>
          ...
          r.status.intValue() match {
            case 200 if isSuccess =>
              ...
              events += WACallbackEvent(messageId, Some(responseBody.payload.message_id.getOrElse("")), requestTracker.destination, WAResponseStatus.Received, requestTracker.clientId, appName, requestTracker.contextId, (responseBody.asMap + ("provider" -> Channel.WA.toString)).getJson, eventTS)
