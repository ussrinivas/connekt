  ...
  override val map: WNSPayloadEnvelope => List[(HttpRequest, WNSRequestTracker)] = message => {
    try {
      ...
      case e: Throwable =>
        ...
        throw new ConnektPNStageException(message.messageId, Set(message.deviceId), InternalStatus.StageError, message.appName, MobilePlatform.WINDOWS, message.contextId, message.client, message.meta, s"WNSDispatcherPrepare-${e.getMessage}", e)
    }
  }
}
