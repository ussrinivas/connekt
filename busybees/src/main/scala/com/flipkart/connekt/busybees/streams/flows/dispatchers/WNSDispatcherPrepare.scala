...
//      ConnektLogger(LogFile.PROCESSORS).trace(s"WNSDispatcher received message: $message")
      ...
      case e: Throwable =>
        ...
        throw new ConnektStageException(message.messageId, message.clientId, Channel.PUSH, Set(message.deviceId), InternalStatus.StageError, message.appName, MobilePlatform.WINDOWS, message.contextId, message.meta, s"WNSDispatcherPrepare-${e.getMessage}", e)
    }
  }
}
