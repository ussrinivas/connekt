  ...
  override val map: (ConnektRequest) => List[ConnektRequest] = input => {
    try {
      ...
      case e: Throwable =>
        ...
        throw new ConnektPNStageException(input.id, input.clientId,input.destinations, InternalStatus.RenderFailure, input.appName, input.platform, input.contextId.orEmpty,  input.meta ++ input.stencilId.map("stencilId" -> _).toMap, s"RenderFlow-${e.getMessage}", e)
    }
  }
}
