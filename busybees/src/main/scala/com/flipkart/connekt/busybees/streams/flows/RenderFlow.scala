  ...
  override val map: (ConnektRequest) => List[ConnektRequest] = input => {
    try {
      ...
      case e: Throwable =>
        ...
        throw new ConnektPNStageException(input.id, input.deviceId, InternalStatus.RenderFailure, input.appName, input.platform, input.contextId.orEmpty, input.client, input.meta ++ input.templateId.map("stencilId" -> _).toMap, s"RenderFlow-${e.getMessage}", e)
    }
  }
}
