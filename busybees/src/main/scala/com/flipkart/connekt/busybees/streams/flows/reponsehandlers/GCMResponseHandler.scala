 ...
 * @version 2/8/16
 */
class GCMResponseHandler(implicit m: Materializer, ec: ExecutionContext) extends PNProviderResponseHandler[(Try[HttpResponse], HttpRequestTrace)] {

  val in = Inlet[(Try[HttpResponse], HttpRequestTrace)]("GCMResponseHandler.In")
