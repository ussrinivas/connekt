  ...
  override def map: ConnektRequest => List[GCMPayloadEnvelope] = message => {
    ...
    try {
      ...
      case e: Exception =>
        ...
        throw new ConnektChannelStageException(message.id, message.clientId, Channel.PUSH,message.destinations, InternalStatus.StageError, message.appName, message.platform, message.contextId.orEmpty, message.meta, "AndroidChannelFormatter::".concat(e.getMessage), e)
    }
  }
}
