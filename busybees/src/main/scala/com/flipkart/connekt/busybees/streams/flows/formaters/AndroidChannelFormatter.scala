  ...
  override def map: ConnektRequest => List[GCMPayloadEnvelope] = message => {
    ...
    try {
      ...
      if (tokens.nonEmpty && ttl > 0) {
        ...
        devicesInfo.map(d => PNCallbackEvent(message.id, message.client, d.deviceId, InternalStatus.TTLExpired, MobilePlatform.ANDROID, d.appName, message.contextId.orEmpty)).persist
      ...
      case e: Exception =>
        ...
        throw new ConnektPNStageException(message.id, message.client, message.deviceId, InternalStatus.StageError, message.appName, message.platform, message.contextId.orEmpty, message.meta, "AndroidChannelFormatter::".concat(e.getMessage), e)
    }
  }
}
