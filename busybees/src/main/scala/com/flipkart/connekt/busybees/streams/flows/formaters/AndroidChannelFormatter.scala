...
class AndroidChannelFormatter(parallelism: Int)(implicit ec: ExecutionContextExecutor) extends NIOFlow[ConnektRequest, GCMPayloadEnvelope](parallelism)(ec) {
  ...
  override def map: ConnektRequest => List[GCMPayloadEnvelope] = message => {
    ...
    try {
      ...
      invalidDeviceIds.map(PNCallbackEvent(message.id, _, InternalStatus.MissingDeviceInfo, MobilePlatform.ANDROID, pnInfo.appName, message.contextId.orEmpty, message.client)).persist
      ...
      if (tokens.nonEmpty && ttl > 0) {
        ...
        List(GCMPayloadEnvelope(message.id, validDeviceIds, pnInfo.appName, message.contextId.orEmpty, message.client, payload, message.meta))
        ...
        devicesInfo.map(d => PNCallbackEvent(message.id, d.deviceId, InternalStatus.TTLExpired, MobilePlatform.ANDROID, d.appName, message.contextId.orEmpty, message.client)).persist
      ...
      case e: Exception =>
        ...
        throw new ConnektPNStageException(message.id, message.deviceId, InternalStatus.StageError, message.appName, message.platform, message.contextId.orEmpty, message.client, message.meta, "AndroidChannelFormatter::".concat(e.getMessage), e)
    }
  }
}
