...
case class PNCallbackEvent(@JsonProperty(required = false) messageId: String,
  ...
  def toBigfootFormat: fkint.mp.connekt.PNCallbackEvent = {
    fkint.mp.connekt.PNCallbackEvent(messageId = messageId, deviceId = deviceId, platform = platform, eventType = eventType, appName = appName, contextId = contextId, client = client, cargo = cargo, timestamp = DateTimeUtils.getStandardFormatted(timestamp))
  }
