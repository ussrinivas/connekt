...
class MessageQueueService(dao: MessageQueueDao, private val defaultTTL: Long = 7.days.toMillis, private val maxRecords: Int = 100) extends TService with Instrumented {
  ...
  def enqueueMessage(appName: String, contactIdentifier: String, messageId: String, expiryTs: Option[Long] = None, read: Option[Boolean] = None, createTS: Option[Long] = None)(implicit ec: ExecutionContext): Future[Int] = {
    dao.enqueueMessage(appName.toLowerCase, contactIdentifier, messageId, expiryTs.getOrElse(System.currentTimeMillis() + defaultTTL), read, createTS).andThen {
