...
import com.flipkart.phantom.client.helper.CommandResponse
...
object BigfootService {
  ...
  def ingest(obj: BaseSchema): Try[Boolean] = {
    ...
    SpecterClient.sendToSpecter(socketClient, request) match {
      case commandResponse: CommandResponse => commandResponse.getResponse match {
        case "SUCCESS" =>
          Success(true)
