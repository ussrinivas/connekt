...
class SubscriptionDao(subscriptionTable:String, jdbcHelper: TMySQLFactory) extends TSubscriptionDao with MySQLDao {
  ...
  override def add(subscription: Subscription): Unit = {
    ...
    try {
      val sql = s"INSERT INTO $subscriptionTable (id, name, source, sink, createdBy, createdTS, lastUpdatedTS, stencilId, shutdownThreshold, active) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
      update(sql, subscription.id, subscription.name, subscription.source, subscription.sink.getJson, subscription.createdBy, subscription.createdTS, subscription.lastUpdatedTS, subscription.stencilId, subscription.shutdownThreshold, subscription.active)
       } catch {
      case e: Exception =>
  ...
  override def update(subscription: Subscription): Unit = {
    implicit val j = mySQLHelper.getJDBCInterface
    try {
      val sql = s"UPDATE $subscriptionTable SET name = ?, source = ?, sink = ?, lastUpdatedTS = ?, stencilId = ?, shutdownThreshold = ?, active= ? WHERE id = ?"
      update(sql, subscription.name, subscription.source, subscription.sink.getJson, subscription.lastUpdatedTS, subscription.stencilId, subscription.shutdownThreshold, subscription.active, subscription.id)
    } catch {
      case e: Exception =>
