...
class SubscriptionDao(subscriptionTable:String, jdbcHelper: TMySQLFactory) extends TSubscriptionDao with MySQLDao {
  ...
  override def update(subscription: Subscription): Unit = {
    implicit val j = mySQLHelper.getJDBCInterface
    try {
      val sql = "UPDATE $subscriptionTable SET name = ?, sink = ?, lastUpdatedTS = ?, eventFilter = ?, eventTransformer = ?, shutdownThreshold = ? WHERE id = ?"
      update(sql,subscription.name, subscription.sink.getJson, subscription.lastUpdatedTS, subscription.eventFilter, subscription.eventTransformer.getJson, subscription.shutdownThreshold)
    } catch {
      case e: Exception =>
