...
class CallbackServiceTest extends CommonsBaseTest {
  ...
  "Callback Service" should "persist Callback Event" in {
    mid = ServiceFactory.getMessageService.saveRequest(pnInfo, "fk-connekt-pn", true).get
    ...
    callbackService.persistCallbackEvent(callBackEvent.messageId, callBackEvent.deviceId, Channel.PUSH, callBackEvent).isSuccess shouldEqual true
  }
