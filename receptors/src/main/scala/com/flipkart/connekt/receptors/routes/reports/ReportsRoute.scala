    ...
    path(ChannelSegment / Segment / "messages" / Segment / Segment / "events") {
      (channel: Channel, appName: String, contactId: String, messageId: String) =>
        authorize(user, "REPORTS") {
          get {
            val events = ServiceFactory.getCallbackService.fetchCallbackEvent(appName, messageId, contactId, channel).get
      ...
      (channel: Channel, appName: String, messageId: String) =>
        authorize(user, "REPORTS") {
          get {
            val events = ServiceFactory.getCallbackService.fetchCallbackEventByMId(appName, messageId, channel).get
