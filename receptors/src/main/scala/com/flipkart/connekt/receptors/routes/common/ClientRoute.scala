                      ...
                      entity(as[AppUserConfiguration]) { userConfig =>
                        ...
                        ServiceFactory.getUserInfoService.getUserInfo(clientName).get match {
                          ...
                          case Some(userInfo) =>
                            ...
                            SyncManager.get().publish(SyncMessage(topic = SyncType.CLIENT_QUEUE_CREATE, List(userConfig.channel.toString, clientName, userConfig.queueName)))
                            ...
                            case UserType.USER =>
                              ServiceFactory.getUserInfoService.getUserInfo(id).get match {
                                ...
                                case Some(userInfo) =>
                                  ...
                                  SyncManager.get().publish(new SyncMessage(SyncType.AUTH_CHANGE, List(id, UserType.USER.toString)))
