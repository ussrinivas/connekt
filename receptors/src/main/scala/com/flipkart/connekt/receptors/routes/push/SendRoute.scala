                        ...
                        onSuccess(Future {
                          profile(s"sendUserPush.$appPlatform.$appName") {
                            ...
                            if (groupedPlatformRequests.nonEmpty) {
                              ...
                              GenericResponse(StatusCodes.Created.intValue, null, SendResponse(s"PN request processed for user $userId.", success.toMap, failure.toList))
                            } else {
                              GenericResponse(StatusCodes.NotFound.intValue, null, Response(s"No device Found for user: $userId.", null))
                            }
                          }
                        }(ioDispatcher)) { result =>
                          complete(result)
