          ...
          (channel: Channel, appName: String, contactId: String, messageId: String) =>
            authorize(user, "REPORTS") {
              get {
                ...
                complete(GenericResponse(StatusCodes.OK.intValue, null, Response(s"Events for $messageId $contactId fetched.", Map(contactId -> events))).respond)
              }
            }
          ...
          (channel: Channel, appName: String, messageId: String) =>
            authorize(user, "REPORTS") {
              get {
                ...
                data match {
                  case None =>
                    complete(GenericResponse(StatusCodes.NotFound.intValue, Map("messageId" -> messageId), Response(s"No Data found for MessageId $messageId", null)).respond)
                  case Some(x) =>
                    complete(GenericResponse(StatusCodes.OK.intValue, Map("messageId" -> messageId), Response(s"Details for $messageId.", data)).respond)
                }
              }
            }
        }
